...
## 1. How to use
1. ✅ 双击 `index.html` 在你的浏览器即可运行。 / Double-click `index.html` to run in your browser.
2. 📺4个样例问题无需消耗 tokens，点击可运行内置演示。/ Sample questions don't need tokens. With no API key, click 4 sample questions to run built-in static demos (no OpenRouter calls, no tokens).
3. 🔑 使用你自己的 tokens，将 OpenRouter API key（[openrouter.ai/keys](https://openrouter.ai/keys)）粘贴到 `index.html` 顶部 “Manage API key”。Key 仅存储在你的浏览器，不会发给任何人。 / Use your own tokens: paste your OpenRouter API key into “Manage API key” at the top. Stored only in your browser.
